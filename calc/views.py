...
# ─── WARD DASHBOARD ───────────────────────────────────────────────────────────
# Called by Dashboard.jsx → dashboardApi.wardStats(ward)
# GET /api/ward-dashboard/?ward=<wardNumber>
...
_ward_dashboard_cache = {}   # { ward_str: {'data': {...}, 'ts': float} }
WARD_CACHE_TTL        = 120  # seconds
...
def api_ward_dashboard(request):
    ...
    if not ward:
        return JsonResponse({'success': False, 'message': 'ward parameter is required.'}, status=400)

    # ── Cache check ───────────────────────────────────────────────────────────
    cached = _ward_dashboard_cache.get(ward)
    if cached and (_time.time() - cached['ts']) < WARD_CACHE_TTL:
        return JsonResponse({'success': True, **cached['data']})

    try:
        db        = get_db()         # original cluster — voter rolls + WardReference
        survey_db = get_survey_db()  # new cluster — SurveyRecords

        ward_int = int(ward) if ward.isdigit() else None

        # ── 1. WardReference — authoritative voter demographics ───────────────
        ward_ref = db['WardReference'].find_one(
            {'number': ward_int} if ward_int is not None else {'number': ward}
        ) or {}

        ward_name    = ward_ref.get('name')        or _WARD_NAMES_MAP.get(ward, f'Ward {ward}')
        district_id  = ward_ref.get('districtId')
        const_id     = ward_ref.get('constituencyId')
        ref_total    = ward_ref.get('totalCount',  0) or 0
        ref_male     = ward_ref.get('maleCount',   0) or 0
        ref_female   = ward_ref.get('femaleCount', 0) or 0
        ref_trans    = ward_ref.get('transCount',  0) or 0
        ref_religion = ward_ref.get('religion',   {}) or {}

        # ── 2. Voter roll (2025) — aggregate for this ward ────────────────────
        # Ward No may be stored as string or int — try both
        ward_filters = [{'Ward No': ward}]
        if ward_int is not None:
            ward_filters.append({'Ward No': ward_int})
        ...
        v_res        = list(db['2025'].aggregate(voter_pipeline))[0]
        total_voters = v_res['total'][0]['n'] if v_res['total'] else ref_total

        gender_map_v = {g['_id']: g['n'] for g in v_res['genders']}
        voter_male   = gender_map_v.get('M', 0) or ref_male
        voter_female = gender_map_v.get('F', 0) or ref_female
        voter_trans  = (gender_map_v.get('O', 0)
                        + gender_map_v.get('T', 0)
                        + gender_map_v.get('TG', 0)) or ref_trans

        rel_code_map   = {'H': 'Hindu', 'M': 'Muslim', 'C': 'Christian',
                          'J': 'Jain',  'B': 'Buddhist', 'S': 'Sikh'}
        rel_map_v      = {g['_id']: g['n'] for g in v_res['religions']}
        voter_religion = {
            name: rel_map_v.get(code, ref_religion.get(name, 0))
            for code, name in rel_code_map.items()
        }

        # ── 3. SurveyRecords — registration stats for this ward ───────────────
        # wardNumber may be stored as string or int — match both
        survey_filters = [{'wardNumber': ward}]
        if ward_int is not None:
            survey_filters.append({'wardNumber': ward_int})

        survey_pipeline = [
            {'$match': {'$or': survey_filters}},
            {
                '$facet': {
                    'total':     [{'$count': 'n'}],
                    'genders':   [{'$group': {'_id': '$gender',   'n': {'$sum': 1}}}],
        ...
        s_res       = list(survey_db['SurveyRecords'].aggregate(survey_pipeline))[0]
        total_reg   = s_res['total'][0]['n'] if s_res['total'] else 0
        house_count = s_res['houses'][0]['n'] if s_res['houses'] else 0

        gender_map_s = {g['_id']: g['n'] for g in s_res['genders']}
        reg_male     = gender_map_s.get('Male',   0)
        reg_female   = gender_map_s.get('Female', 0)

        religions    = ['Hindu', 'Muslim', 'Christian', 'Jain', 'Buddhist', 'Sikh']
        rel_map_s    = {r['_id']: r['n'] for r in s_res['religions']}
        reg_religion = {r: rel_map_s.get(r, 0) for r in religions}

        # ── 4. Coverage ───────────────────────────────────────────────────────
        denom        = total_voters or ref_total or 1
        ...
        result = {
            ...
            # Voter roll stats (from 2025 collection / WardReference)
            'totalVoters':    total_voters,
            'voterMale':      voter_male,
            'voterFemale':    voter_female,
            'voterTrans':     voter_trans,
            'voterReligion':  voter_religion,
            # Survey / registration stats (from SurveyRecords)
            'totalReg':       total_reg,
            'regMale':        reg_male,
            'regFemale':      reg_female,
            'regReligion':    reg_religion,
            'houseCount':     house_count,
            # Coverage map — used by BarChart in Dashboard.jsx
            'wardCoverage':   {ward_name: coverage_pct},
        }

        _ward_dashboard_cache[ward] = {'data': result, 'ts': _time.time()}
